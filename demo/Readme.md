...
mers-ffmpeg supports udp protocol using mpegts format and rtmp broadcasting for streaming. There are 2 example pipelines in the files `stream_mpegts_pipeline.json` and `stream_rtmp_pipeline.json`.

**Notes**

 * Real-time live streaming enconding requires the -re flag enable at input params.
 * RTMP streaming requires the RTMP server IP and path, this should be changed in `stream_rtmp_pipeline.json` prior to send json.

When creating a streaming pipeline, probably it will be required to stop the pipeline, for this use the `DELETE` method on `/media/<service_id>/pipeline/<pipeline_id>`. See the file `stop_stream.sh` for reference.
